...
const shouldFilterDefault = true;
const labelDefault = 'Command Menu';
...
const Command = React.forwardRef<HTMLDivElement, CommandProps>(
  (props, forwardedRef) => {
    ...
    const {
      ...
      onValueChange,
      filter,
      shouldFilter = shouldFilterDefault,
      loop,
    ...
    const context: Context = React.useMemo(
      () => ({
        ...
        filter: () => {
          return propsRef.current.shouldFilter ?? shouldFilterDefault;
        },
        label: label || props['aria-label'] || labelDefault,
    ...
    function sort() {
      ...
      state.current.filtered.groups.forEach((value) => {
        ...
        let max = 0;
        // TODO: check that `items` and `score` are always defined
        items!.forEach((item) => {
